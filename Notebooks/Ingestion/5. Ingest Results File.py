...
results_df = spark.read.schema(results_schema).json(f"{bronze_folder_path}/{v_file_date}/results.json")

# COMMAND ----------

# MAGIC %md
# MAGIC
# MAGIC #### Step2: Select and rename columns as required
...
# results_final_df.write.format("delta").saveAsTable("f1_silver.results")
...
# # results_final_df.write.mode("overwrite").partitionBy('race_id').parquet(f"{silver_folder_path}/results")
...
# MAGIC %sql 
# MAGIC select * from f1_silver.results
